...
let pw = { pushPasswordHistory: () => [] };
try {
  pw = { ...pw, ...require('/opt/app/src/extensions/users-permissions/password') };
} catch (err) {
  strapi && strapi.log && strapi.log.error('[user-service] shared password module not found:', err.message);
}
...
const create = async (attributes) => {
  ...
  // A: a password set on creation (registerAdmin, CLI admin:create-user,
  // bootstrap ADMIN_PASSWORD) must start the aging / history policy like any
  // other password. Without passwordChangedAt the account would never expire.
  if (user.password) {
    user.passwordChangedAt = new Date();
    user.passwordHistory = pw.pushPasswordHistory(null, user.password);
  }
...
const updateById = async (id, attributes) => {
  ...
  if (_.has(attributes, 'password')) {
    const newPassword = attributes.password;

    // M2: any password set through the users service (invite register, admin
    // panel "reset password", CLI reset) must satisfy the same policy as
    // self-service: strength, reuse and breach checks.
    const strengthError = pw.validatePasswordStrength
      ? pw.validatePasswordStrength(newPassword)
      : null;
    if (strengthError) {
      throw new ValidationError(strengthError);
    }

    const currentUser = await strapi.query('admin::user').findOne({ where: { id } });

    if (pw.isPasswordReused && currentUser) {
      const reused = await pw.isPasswordReused(
        newPassword,
        currentUser.passwordHistory,
        currentUser.password
      );
      if (reused) {
        throw new ValidationError('You cannot reuse a recently used password');
      }
    }

    if (pw.checkBreachedPassword) {
      const breached = await pw.checkBreachedPassword(newPassword);
      if (breached) {
        throw new ValidationError(breached);
      }
    }

    const hashedPassword = await getService('auth').hashPassword(newPassword);

    // Bump the JWT version so any previously issued token (with an older jwtVersion) is
    // invalidated. This revokes existing sessions when the password changes.
    const jwtVersion = ((currentUser && currentUser.jwtVersion) || 0) + 1;
    ...
    const updatedUser = await strapi.query('admin::user').update({
      ...
      data: {
        ...
        passwordHistory: pw.pushPasswordHistory(
          currentUser && currentUser.passwordHistory,
          hashedPassword
        ),
      },
      populate: ['roles'],
...
const displayWarningIfUsersDontHaveRole = async () => {
  ...
  if (count > 0) {
    strapi.log.warn(`Some users (${count}) don't have at least one role.`);
  }
};
